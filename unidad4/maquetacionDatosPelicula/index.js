...
window.onload= ()=>{
 ...
 document.getElementById("cargar").addEventListener("click",()=>maquetarPelis(pelis2));
 
 fetch("https://www.omdbapi.com?s=star&apikey=9de3cf92&page=1").then(response => response.json())
 .then(data => {
    maquetarPelis(data.Search)
 });

}
function maquetarPelis(peli){
    for (let peliculas of peli.Search) {
    let divCuadro = document.createElement("div");

    divCuadro.innerHTML = "<img src="+peliculas.Poster+"/>";
